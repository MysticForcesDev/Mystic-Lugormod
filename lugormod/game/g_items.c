...
int MaxAmmo(gentity_t *ent, int ammo){
	return ammoData[ammo].max + (ammoData[ammo].max * PlayerProf_Merc_GetAmmoSkill(ent) * 0.2);
}
//======================================================================
...
#define SHIELD_HEALTH				250             // Lugormod
#define SHIELD_HEALTH_DEC			20		// 25 seconds	
...
// No no count it up Lugormod
extern vmCvar_t g_fixShields; //Lugormod
...
void ShieldThink(gentity_t *self)
{
	...
	{
		//Lugormod
		if (g_fixShields.integer && !gameMode(GM_ALLWEAPONS)) {
			self->health += SHIELD_HEALTH_DEC;
			if (self->health > ceil((float)(SHIELD_HEALTH *2))) {
				self->health = ceil((float)(SHIELD_HEALTH *2));
			}
		} else {
			self->health -= SHIELD_HEALTH_DEC;
		}
	...
	if (!self->parent->client || self->parent->client->pers.connected != CON_CONNECTED) {
		ShieldRemove(self);
	}
...
void ShieldGoSolid(gentity_t *self)
{
	...
	if(tr.startsolid){
		...
		self->think = ShieldThink;
		//Lugormod
		if (g_gametype.integer != GT_FFA || self->genericValue1 != PROF_ADMIN) {
...
void ShieldTouch(gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	//RoboPhred: only in non team gametypes
	//if(g_gametype.integer != GT_SIEGE && 
	//	g_gametype.integer != GT_BATTLE_GROUND &&
	//Ufo: also exclude friends
	if (g_gametype.integer == GT_FFA && (isBuddy(self->parent, other) || PlayerAcc_Friends_IsFriend(self->parent, other->client->sess.Lmd.id))) {
		ShieldGoNotSolid(self);
		return;
	}
}
void ShieldUse(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	if (g_gametype.integer >= GT_TEAM && !gameMode(GM_ALLWEAPONS))
	{ // let teammates through
		// compare the parent's team to the "other's" team
		if (self->parent && ( self->parent->client) && (other->client))
		{
			if (OnSameTeam(self->parent, other))
			{
				...
				other->client->ps.stats[STAT_HOLDABLE_ITEM] = HI_SHIELD;
				ShieldRemove(self);
				return;
			}
		}
...
void CreateShield(gentity_t *ent)
{
	...
	{	// Get solid.
		...
		//Lugormod
		if (g_gametype.integer != GT_FFA || ent->genericValue1 != PROF_ADMIN) {
...
qboolean PlaceShield(gentity_t *playerent)
{
	...
	//Ufo: different approach
	if (g_fixShields.integer == 2)
	{
		gentity_t* ent;
		for (int n = 0; n < MAX_GENTITIES; n++)
		{
			ent = &g_entities[n];
			if (ent && ent->inuse && ent->s.owner == playerent->s.number && ent->s.eType == ET_SPECIAL && ent->s.modelindex == HI_SHIELD)
			{
				Disp(playerent, "You cannot place more than one forcefield at a time.");
				playerent->client->ps.stats[STAT_HOLDABLE_ITEMS] |= (1 << HI_SHIELD);
				return 0;
			}
		}
	}
	...
	//RoboPhred
	int prof = PlayerAcc_Prof_GetProfession(playerent);
	...
	{//room in front
		...
		{
			//Lugormod if we have a shield remove it
			if (g_gametype.integer == GT_FFA && prof != PROF_ADMIN) {
				int i;
				gentity_t *check = g_entities;

				for (i = 0; i < MAX_GENTITIES; i++, check++){
					if (!check->inuse
						|| check->parent != playerent
						|| check->s.eType != ET_SPECIAL
						|| check->s.modelindex != HI_SHIELD) {
							continue;
					}
					ShieldRemove(check);
				}
...
void ItemUse_Binoculars(gentity_t *ent)
{
	...
	*/
	...
	if (ent->client->ps.zoomMode == 0) // not zoomed or currently zoomed with the disruptor
	{
		...
		ent->client->ps.zoomFov = 40.0f; //Lugormod better zoom?
	}
...
//-----------------------------------------------------
{
	...
	count = G_RadiusList( org2, TURRET_RADIUS, self, qtrue, entity_list, qfalse );
	...
	for ( i = 0; i < count; i++ )
	{
		...
		//Lugormod
		//Ufo: also exclude friends
		if (isBuddy(self->parent, target) || PlayerAcc_Friends_IsFriend(self->parent, target->client->sess.Lmd.id)) {
			continue;
		}
...
//---------------------------------
{
	...
	//Ufo: also exclude friends
	else if (isBuddy(ent->parent, ent->enemy) || PlayerAcc_Friends_IsFriend(ent->parent, ent->enemy->client->sess.Lmd.id)) { //Lugormod
		keep = qfalse;
	}
	else
	{
...
{ //create effects?
	...
	//Lugormod don't like jp in duels.
	//Add jetpack duel
...
void ItemUse_Jetpack( gentity_t *ent )
{
	...
	//RoboPhred: Jedi never use jetpack.
	if(ent->client->ps.trueJedi)
		return;

	//Cant jetpack in a vehicle
	if(ent->client->ps.m_iVehicleNum)
		return;

	if(ent->client->ps.forceJumpFlip)
		return;
	...
	if(!ent->client->jetPackOn && ent->client->ps.weaponTime > 0)
		return;
	if(PM_InKnockDown(&ent->client->ps))
		return;

	//RoboPhred
	if(PlayerAcc_Prof_GetProfession(ent) == PROF_ADMIN) {
		if(ent->client->jetPackToggleTime >= level.time)
			return;
		if(!ent->client->jetPackOn) {
			ent->client->jetPackOn = qtrue;
			ent->client->jetPackTime = Q3_INFINITE;
		}
		else {
			ent->client->jetPackOn = qfalse;
			ent->client->jetPackTime = level.time;
		}
	}
	else {
		...
		if(!ent->client->jetPackOn) {
			...
			int burst = Merc_JetpackDefuelBurst(ent);

			if (ent->client->ps.jetpackFuel < burst * 1.5)
				return;

			//Take a burst of fuel to get started.
			ent->client->ps.jetpackFuel -= burst;
		}
		//RoboPhred: Client game draws the red bar in negitive space at 0, keep it at 1.
		else if(ent->client->ps.jetpackFuel <= 1)
			return;
		ent->client->jetPackOn = qtrue;
...
void ItemUse_UseCloak( gentity_t *ent )
{
	...
	{
		//Com_Printf("info: %s cloak toggle time.\n", ent->client->pers.netname);
		return;
	}
	...
	{ //too low on fuel to start it up
		Com_Printf("info: %s has no cloak fuel.\n", ent->client->pers.netname);
		return;
	}

	//RoboPhred
	if(ent->client->Lmd.bodyshieldPower > 0){
		Disp(ent, "^3You cannot use cloak while using a bodyshield.");
		return;
	}

	if ( ent->client->ps.powerups[PW_CLOAKED] )
	{//decloak
		//Com_Printf("info: %s decloaked.\n", ent->client->pers.netname);
		Jedi_Decloak( ent );
	}
	else
	{//cloak
		//Com_Printf("info: %s cloaked.\n", ent->client->pers.netname);
		Jedi_Cloak( ent );
	}
...
gentity_t* HurlItem (gentity_t *ent, const char *name); //Lugormod
...
void ItemUse_UseDisp(gentity_t *ent, int type)
{
	...
	ent->client->tossableItemDebounce = level.time + te->s.time2;

	if (type == HI_HEALTHDISP)
	{
		HurlItem(ent, DISP_HEALTH_ITEM);

		//item = BG_FindItem(DISP_HEALTH_ITEM);
	}
	else
	{
		HurlItem(ent, DISP_AMMO_ITEM);
		...
		if (!tr.startsolid && !tr.allsolid)
		{
			...
			{ //we moved, do some animating
				...
				if (dAng[YAW] > 0.0f)
				{
					...
					G_SetAnim(owner, SETANIM_LEGS, BOTH_STRAFE_LEFT1, aFlags, 0);
				}
				else
				{
					...
					G_SetAnim(owner, SETANIM_LEGS, BOTH_STRAFE_RIGHT1, aFlags, 0);
				}
			}
...
//======================================================================
int Pickup_Ysalamiri (gentity_t *ent, gentity_t *other){
	if (other->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_YSALAMIRI) || PlayerProf_Merc_GetYsalamiriSkill(other) < 1){
		return Pickup_Powerup(ent, other);
	}

	other->client->ps.stats[STAT_HOLDABLE_ITEMS] |= (1 << HI_YSALAMIRI);
	return 60;
}
//======================================================================
...
void Add_Ammo (gentity_t *ent, int weapon, int count){
	int maxammo = MaxAmmo(ent, weapon);
	//RoboPhred: want to give the max ammo, but do not want to overflow
	if(count == Q3_INFINITE){
...
int Pickup_Ammo (gentity_t *ent, gentity_t *other){
	...
	{ //an ammo_all, give them a bit of everything
		if(g_gametype.integer == GT_SIEGE || g_gametype.integer == GT_BATTLE_GROUND)	// complaints that siege tech's not giving enough ammo.  Does anything else use ammo all?
		{
...
===============
*/
//void updateGhost (gentity_t *ent); //Lugormod
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (g_gametype.integer == GT_GHOST && 
		...
		other->client &&
		ent->s.teamowner != other->client->sess.sessionTeam) {
			return;
	}
	...
	if ( other->client->NPC_class == CLASS_ATST || 
		...
		other->client->NPC_class == CLASS_SAND_CREATURE || 
	...
	case IT_WEAPON:
		if (g_gametype.integer == GT_GHOST && other->client) {
			if (!(other->client->ps.stats[STAT_WEAPONS] & (1 << WP_MELEE))) {
				return;
			}
	...
	case IT_HEALTH:
		...
		break;
	case IT_POWERUP:
		//RoboPhred: Pickup_Ysalamiri() seems to be all screwed up, leaving this as origional lmd
		if (ent->item->giTag == PW_YSALAMIRI /*&& PlayerAcc_Prof_GetProfession(ent, PROF_MERC, SK_MERC_YSALAMIRI) > 0*/) {
				respawn = Pickup_Ysalamiri(ent, other);
				predict = qfalse;
				break;
		}
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity ) {
	...
	dropped->s.modelindex2 = 1; // This is non-zero as it's a dropped item
	...
	if ((g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY) && item->giType == IT_TEAM) { // Special case for CTF flags
		...
		&& item->giType == IT_TEAM){
			if (strcmp(dropped->classname, "team_CTF_neutralflag") == 0)
			{
...
void FinishSpawningItem( gentity_t *ent ) {
	...
	if (g_gametype.integer != GT_TEAM && //Lugormod
		g_gametype.integer != GT_CTF &&
		g_gametype.integer != GT_CTY &&
		g_gametype.integer != GT_SABER_RUN && //Lugormod
	...
	VectorSet (ent->r.mins, -8, -8, -0);
	VectorSet (ent->r.maxs, 8, 8, 16);
	//Lugormod this was wrong.
	if (ent->item->giType == IT_HOLDABLE  && ent->item->giTag == HI_EWEB){
		ent->r.mins[2] = -24;
	}
	...
	*/
	if ( ent->spawnflags & ITMSF_SUSPEND ) {
		...
		ent->s.origin[2] += 0.5;//+= 0.1; Lugormod
		ent->r.maxs[2] -= 0.5;//-= 0.1; Lugormod
...
void G_CheckTeamItems( void ) {
	...
	if (g_gametype.integer == GT_SABER_RUN) {
		gitem_t *item;
...
extern vmCvar_t g_autoRandomSpots;
...
void G_SpawnItem (gentity_t *ent, gitem_t *item) {
	...
	if (g_gametype.integer == GT_GHOST && disablesenabled) {
		G_FreeEntity(ent);
		return;
	}
	...
	if ((gameMode(GMF_WITH_JP) 
		...
		&& item->giTag == HI_JETPACK)
		|| (gameMode(GMF_WITH_HOOK)
		&& item->giType == IT_WEAPON
		&& item->giTag == WP_STUN_BATON)
		|| (gameMode(GMF_WITH_TELEPORTER)
		&& item->giType == IT_WEAPON
		&& item->giTag == WP_BRYAR_OLD)
		|| (gameMode(GM_ANY)
		&& item->giType == IT_POWERUP)
		|| ((gameMode(GM_INSTGIB)
		|| gameMode(GM_INSTDIS)
		|| gameMode(GM_BOXING_ARENA))
		&& item->giType != IT_HOLDABLE
		&& item->giType != IT_TEAM)
	...
	if (g_gametype.integer != GT_JEDIMASTER	&& g_gametype.integer != GT_REBORN && !gameMode(GM_ROCKET_ARENA) && !gameMode(GM_SNIPER_ARENA))
	{
		if (item->giType == IT_WEAPON && ((disablesenabled &&  wDisable &&  wDisable & (1 << item->giTag))))
		{
			//place random_spot
			if (g_autoRandomSpots.integer) {
				SP_random_spot(ent);
		...
		if (g_gametype.integer != GT_SIEGE && ((disablesenabled && (g_pickupDisable.integer & (1 << item->giType))))) {
			//place random_spot
			if (g_autoRandomSpots.integer) {
				SP_random_spot(ent);
			}
	  ...
	  gameMode(GM_SNIPER_ARENA) ) &&
	  (item->giType == IT_WEAPON || item->giType == IT_AMMO)) {
	  //more ammo in arena mode
	  item->quantity *= 2;
	  }
	...
	RegisterItem( item );
	SaveRegisteredItems(); //Lugormod experiment
	...
	if ( item->giType == IT_POWERUP ) {
		...
		//RoboPhred: annoys the hell out of servers that dont know how to set this.
		if(ent->Lmd.spawnData)
			defaultVal = "1";
		else
			defaultVal = "0";
		G_SpawnFloat( "noglobalsound", defaultVal, &ent->speed);
...
void G_RunItem( gentity_t *ent ) {
	...
	if ((contents & CONTENTS_LAVA && ent->clipmask & CONTENTS_LAVA) 
		&& !ent->isSaberEntity) { //Lugormod I dont want stuff in lava
			ent->s.pos.trDelta[2] += 20;
			//ent->r.currentOrigin[2] += 5;
			if (ent->s.pos.trType == TR_STATIONARY) {
				ent->s.pos.trType = TR_GRAVITY;
			}
	} 
	//else if (ent->clipmask && !ent->isSaberEntity) {
	//        ent->clipmask |= CONTENTS_LAVA;
	//}
